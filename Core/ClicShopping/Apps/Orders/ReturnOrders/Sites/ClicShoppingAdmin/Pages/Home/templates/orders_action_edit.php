...
if (isset($_GET['oID'])) {
  $Qstatus = $CLICSHOPPING_ReturnOrders->db->prepare('select return_action_id,
                                                                 language_id,
                                                                 name
                                                          from :table_return_orders_action
                                                          where language_id = :language_id
                                                          and return_action_id = :return_action_id
                                                        ');

  $Qstatus->bindInt(':language_id', $CLICSHOPPING_Language->getId());
  $Qstatus->bindInt(':return_action_id', $_GET['oID']);
  $Qstatus->execute();

  $status = $Qstatus->fetch();

  $return_action_id = $status['return_action_id'];
...
<div class="contentBody">
      ...
      <div class="card card-block headerCard">
        <div class="row">
          ...
          <span class="col-md-4 text-end">
            <?php
            echo HTML::form('status_orders_status', $CLICSHOPPING_ReturnOrders->link('OrdersAction&' . $action . '&oID=' . $return_action_id));
            echo HTML::button($CLICSHOPPING_ReturnOrders->getDef('button_update'), null, null, 'success') . ' ';
            echo HTML::button($CLICSHOPPING_ReturnOrders->getDef('button_cancel'), null, $CLICSHOPPING_ReturnOrders->link('OrdersAction'), 'warning');
            ?>
          </span>
        </div>
      </div>
  ...
  <div class="adminformTitle">
    ...
    <?php
    /*
  if (DEFAULT_return_action_id != $return_action_id) {
    ?>
