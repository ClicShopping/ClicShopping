...
<div class="contentBody">
      ...
      <div class="card card-block headerCard">
        <div class="row">
          ...
          <span
            ...
            <?php
            echo HTML::button($CLICSHOPPING_ReturnOrders->getDef('button_insert'), null, $CLICSHOPPING_ReturnOrders->link('OrdersStatusEdit'), 'success');
            echo '&nbsp;';
  ...
  <table
    ...
    <tbody>
    <?php
    $QReturnStatus = $CLICSHOPPING_ReturnOrders->db->prepare('select SQL_CALC_FOUND_ROWS return_status_id,
                                                                                           language_id,
                                                                                           name
                                                                from :table_return_orders_status
                                                                where language_id = :language_id
                                                                order by return_status_id
      ...
      while ($QReturnStatus->fetch()) {
        ...
        <td class="text-end">
          <div class="btn-group d-flex justify-content-end" role="group" aria-label="buttonGroup">
            <?php
            echo '<a href="' . $CLICSHOPPING_ReturnOrders->link('OrdersStatusEdit&page=' . (int)$page . '&oID=' . $QReturnStatus->valueInt('return_status_id')) . '"><h4><i class="bi bi-pencil" title="' . $CLICSHOPPING_ReturnOrders->getDef('icon_edit') . '"></i></h4></a>';
            echo '&nbsp;';
            echo '<a href="' . $CLICSHOPPING_ReturnOrders->link('OrdersStatus&DeleteConfirm&page=' . (int)$page . '&oID=' . $QReturnStatus->valueInt('return_status_id')) . '"><h4><i class="bi bi-trash2" title="' . $CLICSHOPPING_ReturnOrders->getDef('image_delete') . '"></i></h4></a>';
            ?>
          </div>
        </td>
        </tr>
