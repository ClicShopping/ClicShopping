...
$CLICSHOPPING_Recommendations_Config = Registry::get('RecommendationsAdminConfig' . $current_module);
...
<div class="contentBody">
  ...
  <?php
  if ($CLICSHOPPING_Recommendations_Config->is_installed === true) {
    ?>
    <form name="MarketingRecommendationsConfigure"
          action="<?php echo $CLICSHOPPING_Recommendations->link('Configure&Process&module=' . $current_module); ?>"
          method="post">

      <div class="mainTitle">
        <?php echo $CLICSHOPPING_Recommendations->getConfigModuleInfo($current_module, 'title'); ?>
      </div>
      ...
      <div class="col-md-12">
        ...
        if ($CLICSHOPPING_Recommendations->getConfigModuleInfo($current_module, 'is_uninstallable') === true) {
          echo '<span class="float-end">' . HTML::button($CLICSHOPPING_Recommendations->getDef('button_dialog_uninstall'), null, '#', 'warning', ['params' => 'data-bs-toggle="modal" data-bs-target="#ppUninstallModal"']) . '</span>';
        }
        ?>
      </div>
    </form>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4
                class="modal-title"><?php echo $CLICSHOPPING_Recommendations->getDef('dialog_uninstall_title'); ?></h4>
            </div>
            <div class="modal-body">
              <?php echo $CLICSHOPPING_Recommendations->getDef('dialog_uninstall_body'); ?>
            </div>
            <div class="modal-footer">
              <?php echo HTML::button($CLICSHOPPING_Recommendations->getDef('button_delete'), null, $CLICSHOPPING_Recommendations->link('Configure&Delete&module=' . $current_module), 'danger'); ?>
              <?php echo HTML::button($CLICSHOPPING_Recommendations->getDef('button_uninstall'), null, $CLICSHOPPING_Recommendations->link('Configure&Uninstall&module=' . $current_module), 'danger'); ?>
              <?php echo HTML::button($CLICSHOPPING_Recommendations->getDef('button_cancel'), null, '#', 'warning', ['params' => 'data-bs-dismiss="modal"']); ?>
            </div>
          </div>
        </div>
      </div>
