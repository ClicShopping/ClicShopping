...
class Install extends \ClicShopping\OM\Domains\ConfigureActionsAbstract
{
  ...
  private static function installDbMenuAdministration(): void
  {
    ...
    $Qcheck = $CLICSHOPPING_Db->get('administrator_menu', 'app_code', ['app_code' => 'app_tools_modules_service']);

    if ($Qcheck->fetch() === false) {

      $sql_data_array = ['sort_order' => 2,
        'link' => 'index.php?A&Tools\ServiceAPP&ServiceAPP',
        'image' => 'service.png',
        'b2b_menu' => 0,
        'access' => 1,
        'app_code' => 'app_tools_modules_service'
      ];
      ...
      for ($i = 0, $n = \count($languages); $i < $n; $i++) {
        ...
        $sql_data_array = ['label' => $CLICSHOPPING_ServiceAPP->getDef('title_menu')];
