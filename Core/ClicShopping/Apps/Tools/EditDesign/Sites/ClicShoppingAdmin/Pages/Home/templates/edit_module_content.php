...
// Merge-aware read resolution: active theme then Default. A Default-only module
// template can thus be opened; saving it creates an override in the theme.
$root = CLICSHOPPING::getConfig('dir_root', 'Shop');

$candidates = [
  $root . $CLICSHOPPING_Template->getDynamicTemplateDirectory() . '/modules/' . $directory_selected . '/content/' . $filename_selected,
  $root . 'sources/template/Default/modules/' . $directory_selected . '/content/' . $filename_selected,
];
