...
$CLICSHOPPING_Antispam_Config = Registry::get('AntispamAdminConfig' . $current_module);
...
<div class="contentBody">
  ...
  if ($CLICSHOPPING_Antispam_Config->is_installed === true) {
    ?>
    <form name="ConfigurationAntispamConfigure"
          action="<?php echo $CLICSHOPPING_Antispam->link('Configure&Process&module=' . $current_module); ?>"
          method="post">

      <div class="mainTitle">
        <?php echo $CLICSHOPPING_Antispam->getConfigModuleInfo($current_module, 'title'); ?>
      </div>
      ...
      <div class="col-md-12">
        <?php
        echo HTML::button($CLICSHOPPING_Antispam->getDef('button_save'), null, null, 'success');

        if ($CLICSHOPPING_Antispam->getConfigModuleInfo($current_module, 'is_uninstallable') === true) {
          echo '<span class="float-end">' . HTML::button($CLICSHOPPING_Antispam->getDef('button_dialog_uninstall'), null, '#', 'warning', ['params' => 'data-bs-toggle="modal" data-bs-target="#ppUninstallModal"']) . '</span>';
        }
        ?>
      </div>
    </form>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4 class="modal-title"><?php echo $CLICSHOPPING_Antispam->getDef('dialog_uninstall_title'); ?></h4>
            </div>
            ...
            <div class="modal-footer">
              ...
              <?php echo HTML::button($CLICSHOPPING_Antispam->getDef('button_cancel'), null, '#', 'warning', ['params' => 'data-bs-dismiss="modal"']); ?>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-md-12 mainTitle">
      <strong><?php echo $CLICSHOPPING_Antispam->getConfigModuleInfo($current_module, 'title'); ?></strong></div>
    <div class="adminformTitle">
      <div class="row">
        <div class="mt-1"></div>
        <div class="col-md-12">
          <div><?php echo $CLICSHOPPING_Antispam->getConfigModuleInfo($current_module, 'introduction'); ?></div>
          <div class="mt-1"></div>
          <div><?php echo HTML::button($CLICSHOPPING_Antispam->getDef('button_install_title', ['title' => $CLICSHOPPING_Antispam->getConfigModuleInfo($current_module, 'title')]), null, $CLICSHOPPING_Antispam->link('Configure&Install&module=' . $current_module), 'warning'); ?></div>
        </div>
      </div>
    </div>
    <?php
