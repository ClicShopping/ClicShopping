...
class GptRetailersOpenAI extends GptRetailers
{
  ...
  {
    parent::__construct();
    $this->deliveryEarliest = defined('CLICSHOPPING_APP_ECOMMERCE_ACP_DELIVERY_EARLIEST') ? CLICSHOPPING_APP_ECOMMERCE_ACP_DELIVERY_EARLIEST : '+3 days';
    $this->deliveryLatest =  defined('CLICSHOPPING_APP_ECOMMERCE_ACP_DELIVERY_LATEST') ? CLICSHOPPING_APP_ECOMMERCE_ACP_DELIVERY_LATEST : '+5 days';
    $this->deliveryDefault = defined('CLICSHOPPING_APP_ECOMMERCE_ACP_DEFAULT_DELIVERY') ? CLICSHOPPING_APP_ECOMMERCE_ACP_DEFAULT_DELIVERY : '3-5 jours ouvrés';
  }
    ...
    private function normalizeAddressToOpenAI(array $address): array
    {
        ...
        return [
            'name' => $address['name'] ?? '',
            'line_one' => $address['line_one'] ?? $address['street_address'] ?? $address['address'] ?? '',
            'line_two' => $address['line_two'] ?? $address['suburb'] ?? $address['line2'] ?? '',
            'city' => $address['city'] ?? '',
            'state' => $address['state'] ?? '',
    ...
    {
        return [
            'name' => trim(($buyer['first_name'] ?? $buyer['firstname'] ?? '') . ' ' . ($buyer['last_name'] ?? $buyer['lastname'] ?? '')),
            'email' => $buyer['email'] ?? $buyer['email_address'] ?? '',
            'phone_number' => $buyer['phone_number'] ?? $buyer['phone'] ?? $buyer['telephone'] ?? ''
        ];
    }
    ...
    private function getDeliveryTimesFromModule(string $moduleCode): array
    {
        try {
            ...
            return [
                'earliest' => date('c', strtotime($this->deliveryEarliest)),
                'latest' => date('c', strtotime($this->deliveryLatest))
            ];
        }
    }
    ...
    private function getDefaultDeliveryTimesByModuleType(string $moduleCode): array
    {
        ...
        return [
            'earliest' => $this->deliveryEarliest,
            'latest' => $this->deliveryLatest
        ];
    }
    ...
    private function buildFreeShippingOption(): array
    {
        ...
        return [
            ...
            'carrier' => 'Livraison standard gratuite',
            'earliest_delivery_time' => $deliveryTimes['earliest'],
            'latest_delivery_time' => $deliveryTimes['latest'],
            'subtotal' => 0,
            'tax' => 0,
    ...
    private function buildDefaultFulfillmentOptions(): array
    {
        return [
            [
                ...
                'subtitle' => $this->deliveryDefault,
                'carrier' => 'Livraison standard',
                'earliest_delivery_time' => date('c', strtotime($this->deliveryEarliest)),
                'latest_delivery_time' => date('c', strtotime($this->deliveryLatest)),
    ...
    private function buildMessages(array $lineItems = []): array
    {
        ...
        if (empty($messages)) {
            $messages[] = [
                ...
                'content_type' => 'plain',
                'content' => 'Livraison gratuite à partir de 50€'
            ];
        }
    ...
    private function buildLinks(): array
    {
        ...
        return [
            ...
            [
                'type' => 'privacy_policy',
                'url' => $baseUrl . 'index.php?Info&Content&pagesId=5'
            ],
            [
    ...
    private function extractCustomerDataFromSession(array $session, array $input): array
    {
        ...
        return [
            'firstname' => $buyer['first_name'] ?? $buyer['firstname'] ?? 'OpenAI',
            'lastname' => $buyer['last_name'] ?? $buyer['lastname'] ?? 'Customer',
            'email_address' => $buyer['email'] ?? 'customer@openai.com',
            'telephone' => $buyer['phone_number'] ?? $buyer['phone'] ?? '',
    ...
    {
        return CLICSHOPPING::link('Shop/index.php', 'Account&HistoryInfo&order_id=' . $orderId);
    }
}
