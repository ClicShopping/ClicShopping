<?php
  ...
  use ClicShopping\Apps\AI\Ecommerce\Classes\ClicShoppingAdmin\SEO\SeoDescriptionFormatter;
  ...
  class SeoOptimizationAgent implements ActorAgentInterface
  {
    ...
    private function generateDescription(array $vars): string
    {
      ...
      $raw = trim($this->llm->generateResponse(
        $this->prompts->getEnrichedDescriptionPrompt($descVars),
        // A few extra tokens over the previous budget to cover the JSON
        // envelope overhead ({"body_html":..., "specs":[...]}) on top of the
        // same amount of prose + specs content as before.
        ['maxTokens' => 1000, 'temperature' => 0.35]
      ));

      return $this->buildDescriptionFromLlmOutput($raw);
    }

    /**
     * The enriched-description prompt now asks the LLM for a JSON envelope
     * {"body_html": "<p>...</p>...", "specs": [{"label":"...", "value":"..."}]}
     * instead of hand-formatted HTML, so the technical-specifications
     * <table> is built HERE, deterministically, in PHP — never by the LLM.
     *

     * @param string $raw Raw LLM response for the enriched-description prompt.
     * @return string Final description: normalised body paragraphs followed
     *                 by the specs table (if any), as a single HTML string.
     */

    private function buildDescriptionFromLlmOutput(string $raw): string
    {
      ...
      // The LLM frequently emits raw newlines / control characters inside the
      // JSON string values (notably body_html). Those bytes are illegal in JSON,
      // so json_decode() fails and the whole envelope would fall back to being
      // rendered as raw "{ \"body_html\": ... }" text in the description. Strip
      // the illegal control bytes first — they carry no formatting once the <p>
      // structure is rebuilt below. (ASCII-only class, byte-wise on purpose so a
      // truncated multibyte sequence can never null the whole string.)
      ...
      $bodyHtml = $decoded['body_html'] ?? $decoded['description'] ?? '';

      if (!is_string($bodyHtml) || trim($bodyHtml) === '') {
        return $formatter->format($raw);
      }

      $bodyHtml = html_entity_decode($bodyHtml, ENT_QUOTES | ENT_HTML5, 'UTF-8');

      // The model routinely mis-places the <p> tags: when it wraps the whole JSON
      // envelope in <p>...</p>, body_html loses its own opening <p> / closing </p>
      // and the tags left inside are unbalanced. SeoDescriptionFormatter's Case-1
      // "<p>...</p>" matcher then silently drops every unbalanced block (observed:
      // 2 of 3 paragraphs lost). Strip the model's <p> tags — replacing each with a
      // space so adjacent sentences never fuse ("charge.The") — and let the
      // formatter rebuild clean, balanced paragraphs deterministically.
      $bodyHtml = preg_replace('#</?p\b[^>]*>#i', ' ', $bodyHtml);
      $body = $formatter->format(trim($bodyHtml));


      $table = $this->buildSpecsTable(
        is_array($decoded['specs'] ?? null)
          ? $decoded['specs']
          : []
      );
    ...
    /**
     * Deterministically builds a <table> from the "specs" array returned by
     * the LLM. Every label/value is escaped; rows with an empty label or
     * value are skipped rather than emitting a blank <tr>.
     *
     * @param array $specs List of ['label' => string, 'value' => string].
     * @return string The <table>...</table> HTML block, or '' if there is
     *                nothing to show.
     */
    private function buildSpecsTable(array $specs): string
    {
      $rows = [];
